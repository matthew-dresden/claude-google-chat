...
| `rawmessage.py` | **Pure accessors for raw Chat `messages.list` resources** — `sender_type`/`is_human_message` (loop-prevention sender gating), `sender_email` (owner gating), and `thread_key` (in-thread replies). Single source of truth shared by `listener.py` and `serve.py` (DRY). |
...
| `serve.py` | **Always-listening responder** (`cgc serve`). One resilient loop (over `PollLoop`) polling the space as the app, one responder per new owner message, structured replies posted (threaded) via the Chat API. Inherits the loop's transient-error survival, fatal-auth fail-fast, consecutive-error bound, and durable `state_file` resume. Configurable `trigger_prefix`/`poll_interval`/`listen_timeout`/`owner_email`; fetcher/poster/responder/state-store injectable for tests. |
